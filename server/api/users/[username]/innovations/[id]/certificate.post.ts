...
// 已审核通过的大创成果可以补充证书材料：只改证书相关字段，成果本身的 status 保持不变。
export default defineEventHandler(async (event) => {
  ...
  if (current.status !== "approved") {
    throw createError({
      statusCode: 400,
      statusMessage: "只有审核通过的大创成果才能补充证书",
    });
  }
  ...
  const [updated] = await db
    ...
    .where(
      and(eq(schema.innovations.id, id), eq(schema.innovations.userId, user.id)),
    )
    .returning();
